//! Implementing a custom Processor: a simple waveshaper (soft-clip distortion).
//!
//! Run with: `cargo run -p trem --example custom_processor`

use trem::dsp::{Oscillator, Waveform};
use trem::graph::{
    Graph, ParamDescriptor, ParamFlags, ParamUnit, ProcessContext, Processor, ProcessorInfo, Sig,
};
...
fn main() {
    ...
    let info = ws.info();
    println!("Custom processor: {}", info.name);
