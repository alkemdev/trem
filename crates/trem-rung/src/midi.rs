...
//! - **Velocity** `1..=127` → `velocity = vel / 127.0`; `0` treated as note-off.

use crate::{BeatTime, Clip, ClipNote, NoteMeta, Provenance, RungError, RungFile};
        ...
        for ev in track {
            ...
            match message {
                MidiMessage::NoteOn { key, vel } => {
                    ...
                    if v == 0 {
                        // Key-off convention
                        if let Some(p) = open.remove(&(ch, k)) {
                            push_note(
                                &mut notes,
                                &mut next_id,
                                ppq,
...
pub fn import_midi_file(bytes: &[u8], opts: MidiImportOptions) -> Result<RungFile, RungError> {
    ...
    file.provenance = Some(Provenance {
        ...
        mapping: Some(
            "class=midi_key+class_offset voice=channel beat=tick/ppqn (quarter=1 beat)".into(),
        ),
    });
    Ok(file)
}
