...
mod graph_process {
    ...
    fn simple_chain_no_events(bencher: Bencher) {
        ...
        g.run(BLOCK, SR, &[note_on(0, 440.0, 0)]).unwrap();
        bencher.bench_local(|| g.run(BLOCK, SR, &[]).unwrap());
    }
    ...
    fn simple_chain_with_note(bencher: Bencher) {
        ...
        bencher.bench_local(|| g.run(BLOCK, SR, &events).unwrap());
    }
