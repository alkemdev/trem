...
impl KickSynth {
    /// Normalized velocities below this are raised so the pitch sweep still starts above the fundamental.
    const MIN_SWEEP_VELOCITY: f64 = 0.3;
    ...
    fn trigger(&mut self, velocity: f64) {
        ...
        self.freq = 150.0 * vel.max(Self::MIN_SWEEP_VELOCITY);
        self.amp = vel;
        self.phase = 0.0;
    }
...
mod tests {
    ...
    fn run_synth(synth: &mut dyn Node, voice: u32, frames: usize) -> Vec<f32> {
        let mut channels = vec![vec![0.0f32; frames]];
        let events = vec![trem::event::TimedEvent {
            sample_offset: 0,
