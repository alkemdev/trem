...
1. ~~**`ClassIndex` alias + `ClipNote` + `Clip` + serde**~~ — shipped as **`trem::rung`** (crate **`trem`**, feature **`rung`**), see §11.  
...
- **MIDI export** — later; **import** exists via **`trem::rung::midi`** (crate feature **`midi`**, §11).  
- MPE, tempo maps, automation lanes, automatic clip→[`Tree`](crates/trem/src/tree.rs) in v1.

---

## 11. Interchange: **Rung** (core module + file format)

The clip model is a **first-class format**, not only an in-memory shape for trem:

| Piece | Location |
|--------|----------|
| Rust API | **`trem::rung`** (package **`trem`**, features **`rung`** / **`midi`**) |
| Source | **`crates/trem/src/rung/`** |
...
Feature **`midi`** on **`trem`** (implies **`rung`**). Default rules (documented in `crates/trem/src/rung/midi.rs`):
...
Anything that consumes/produces `RungFile` JSON is a valid pipeline stage: quantize classes, merge voices, strip meta, round-trip through another notation tool, etc. Keeping Rung behind optional **`trem`** features (`serde`, `serde_json`, `num-rational`, optional `midly`) keeps the default core build lean.
